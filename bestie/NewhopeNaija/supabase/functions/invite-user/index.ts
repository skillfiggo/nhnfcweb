...
serve(async (req) => {
  ...
  try {
    ...
    // ── KEY STEP: Delete any existing stale auth user with this email ──
    // This allows us to always send a completely fresh invite with no conflicts.
    // The player's profile row in the `profiles` table is NOT deleted — data is preserved.
    ...
    if (existingAuthUser) {
      console.log(`Found existing auth user for ${email} (id: ${existingAuthUser.id}). Deleting stale record to issue fresh invite...`);
      const { error: deleteError } = await supabaseAdmin.auth.admin.deleteUser(existingAuthUser.id);
      if (deleteError) {
        throw new Error('Could not reset the invitation: ' + deleteError.message);
      }
      console.log('Stale auth record deleted. Proceeding with fresh invitation...');
    }
    ...
    if (RESEND_API_KEY) {
      console.log('Resend API key found. Using Resend for invitation email.');

      // Generate a fresh invite link — no conflict possible now
                    ...
                    <table width="600" cellpadding="0" cellspacing="0" border="0" style="background-color: #ffffff; border-radius: 12px; overflow: hidden; box-shadow: 0 8px 30px rgba(0,0,0,0.08); max-width: 600px; margin: 0 auto;">
                      ...
                      <tr>
                        <td style="background-color: #001f5b; padding: 40px 30px; text-align: center;">
                          <h1 style="color: #ffffff; font-size: 28px; line-height: 1.2; margin: 0; font-weight: 800; text-transform: uppercase; letter-spacing: 1px;">
                            Welcome to <span style="color: #cc0000;">NewHope</span> Naija FC
                          </h1>
                        </td>
                      </tr>
                      ...
                      <tr>
                        <td style="padding: 40px 40px 30px; background-color: #ffffff;">
                          <h2 style="color: #001f5b; font-size: 22px; font-weight: 600; margin: 0 0 20px; text-align: center;">
                            You've Been Invited to the Club! ⚽
                          </h2>

                          <p style="color: #4a5568; font-size: 16px; line-height: 1.6; margin: 0 0 20px; text-align: center;">
                            We are excited to invite you to join the official <strong>NewHope Naija FC</strong> digital platform.
                            Your player profile is ready to be claimed.
                          </p>

                          <p style="color: #4a5568; font-size: 16px; line-height: 1.6; margin: 0 0 30px; text-align: center;">
                            Click the button below to securely set up your password, access your personalized dashboard,
                            and stay updated with the latest club news, match fixtures, and your personal performance stats.
                          </p>
